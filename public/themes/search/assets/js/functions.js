 ...
 * @return  string
 */
function loadPage(argUrl, argType, argParams, options = {
    loadingBar: true
}) {
...
function search() {
    var searchInput = jQuery('#search-input');
    loadPage(searchInput.data('search-url') + searchInput.data('suggestions-path'), 1, {
        q: searchInput.val(),
        searchType: searchInput.data('search-path'),
        token_id: searchInput.data('token-id')
    }, {
        loadingBar: false
    });
}
...
function openPane(target, options = {
    ...
    jQuery('.preview-pane').css({
        'top': (curTopPos + jQuery(target).outerHeight() + 10) + 'px'
    })
    ...
    jQuery('#pane-url-image').css({
        'width': imgRatio.width + 'px',
        'height': imgRatio.height + 'px',
        'display': 'block'
    });
    // Set the preview pane arrow
    jQuery('.preview-pane-arrow').css({
        'left': (curLeftPos + (jQuery(target).outerWidth(true) / 2)) - 15 + 'px'
    });

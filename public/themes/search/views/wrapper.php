...
<head>
    ...
    <?php if($data['settings']['search_privacy']): ?>
    <meta name="referrer" content="same-origin">
    <?php endif ?>

    <link href="<?=$data['url']?>/<?=UPLOADS_PATH?>/brand/<?=$data['settings']['favicon']?>" rel="icon">

    <?php foreach(['jquery.flex-images', 'style'] as $name): ?>
    <link href="<?=$data['url']?>/<?=$data['theme_path']?>/<?=$data['settings']['site_theme']?>/assets/css/<?=$name?>.css" rel="stylesheet" type="text/css">
    <?php endforeach ?>

    <?php foreach(['jquery', 'jquery.flex-images', 'dragscroll', 'functions'] as $name): ?>
    <script type="text/javascript" src="<?=$data['url']?>/<?=$data['theme_path']?>/<?=$data['settings']['site_theme']?>/assets/js/<?=$name?>.js"></script>
    <?php endforeach ?>

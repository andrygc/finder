...
<form action="<?=$data['url']?>/admin/ads" method="post">
    ...
    <label for="i_ads_safe"><?=$lang['ads_safe']?></label>
    <select name="ads_safe" id="i_ads_safe">
        <option value="0"<?=($data['site_settings']['ads_safe'] == 0 ? ' selected' : '')?>><?=$lang['off']?></option>
        <option value="1"<?=($data['site_settings']['ads_safe'] > 0 ? ' selected' : '')?>><?=$lang['on']?></option>
    </select>
